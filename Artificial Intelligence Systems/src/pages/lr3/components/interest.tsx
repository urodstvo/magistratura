...
import { interestData } from "../weights";

const chartConfig = {} satisfies ChartConfig;

export const InterestQuestion = ({ id }: { id: number }) => {
      ...
      {payload
        ...
        .map((entry, index) => (
          <li key={`item-${index}`} className="flex items-center gap-1">
            ...
            {entry.value === "none" && "Совсем не интересно"}
            {entry.value === "low" && "Немного интересно"}
            {entry.value === "med" && "Интересно"}
            {entry.value === "high" && "Очень интересно"}
          </li>
        ))}
    </ul>
  );
