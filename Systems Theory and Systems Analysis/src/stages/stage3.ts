...
const $y1 = (t: number, x1: number, x2: number, y1: number, y2: number, alpha: number[]) => {
    const m1 = alpha[6] + Km * Mp;
    const k3 = alpha[4];
    const x = x1 - x2;

    return (G(t, alpha) - f(x, alpha) * (y1 - y2) - mu * k3 * y1 - p(x, alpha) - k3 * x1) / m1;
};

const $y2 = (t: number, x1: number, x2: number, y1: number, y2: number, alpha: number[]) => {
    const m2 = alpha[7];
    const k2 = alpha[3];
    const x = x1 - x2;

    return (-G(t, alpha) + f(x, alpha) * (y1 - y2) - mu * k2 * y2 + p(x, alpha) - k2 * x2) / m2;
};
...
const solution = () => {
    ...
    for (let i = 1; i <= n; i++) {
        ...
        const y1_new = y1 + h * $y1(t, x1, x2, y1, y2, alpha);
        const y2_new = y2 + h * $y2(t, x1, x2, y1, y2, alpha);
